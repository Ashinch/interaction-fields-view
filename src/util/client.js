...
class AwaitingConfirm {
  ...
  applyServer(client, revision, operation) {
    ...
    //  to the client's
    //  current document)
    const op1 = this.outstanding
    const op2 = operation
    if (op1.baseLength !== op2.baseLength) return
    const pair = TextOperation.transform(op1, op2)
...
class StaleWithBuffer {
  constructor(acknowledged, buffer, client, revision) {
    this.acknowlaged = acknowledged
    this.buffer = buffer
    this.client = client
  ...
  applyOperations(client, head, operations) {
    ...
    for (let i = 0; i < operations.length; i++) {
      ...
      let op1 = this.acknowlaged
      let op2 = op
      if (op1.baseLength !== op2.baseLength) return
      const pair1 = transform(op1, op2)
      op1 = this.buffer
      op2 = pair1[1]
      if (op1.baseLength !== op2.baseLength) return
  ...
  getOperations() {
    this.client.getOperations(this.client.revision, this.revision - 1) // acknowledged is the one at revision
    return this
  };
}

class Stale {
  constructor(acknowledged, client, revision) {
    this.acknowlaged = acknowledged
    this.client = client
    this.revision = revision
  }
...
export default class Client {
  ...
  applyClient(operation) {
    console.log(this.state, `applyClient: revision=${operation}`)
    this.state = this.state?.applyClient(this, operation)
  };

  // Call this method with a new operation from the server
  applyServer(revision, operation) {
    console.log(this.state, `applyServer: revision=${revision}, operation=${operation}`)
  ...
  serverAck(revision) {
    console.log(this.state, `serverAck: revision=${revision}`)
